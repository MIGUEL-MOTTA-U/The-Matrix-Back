...
const gameController = new GameController(matchRepository, userRepository, gameService);
...
describe('GameController', () => {
  ...
  it('should handle a WebSocket connection and start a match', async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith('The User: user1 is connected to the match match1');
  });
  ...
  it('should handle validation errors and close the socket', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith('An error occurred on Socket message request...');
